...
if __name__ == '__main__':
    ...
    name = f'{args.datasplit}_p{args.top_p}_k{args.top_k}_t{args.temp}_ep{args.top_ep}_tm{args.top_ep_tm}_cs{args.cs_alpha}_typical{args.typical_p}_seed{args.seed}'
    ...
    if os.path.isfile(f'{folder_name}/sample_{name}.p'):
        ...
        sample_fn = gen_utils.create_sample_fn(model, args.max_len,
            top_p=args.top_p, top_k=args.top_k, temperature=args.temp, top_ep=args.top_ep, top_ep_tm=args.top_ep_tm, cs_alpha=args.cs_alpha, typical_p=args.typical_p
        )
        t1 = time.time()
        
        samples, is_completed = gen_utils.get_samples_from_sample_fn(
            sample_fn, ds_tokens, tokenizer.eos_token_id, tokenizer, device,
