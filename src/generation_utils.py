...
def generate_text_from_recalibrated_model(
        ...
        top_ep: Optional[float] = None,
        top_ep_tm: Optional[float] = None,
    ...
    if num_beams > 1:
        ...
        output = _generate_no_beam_search(model, input_ids,
                                          ...
                                          do_sample=do_sample, temperature=temperature, top_k=top_k, top_p=top_p, top_ep=top_ep, top_ep_tm=top_ep_tm,
...
def _generate_no_beam_search(model, input_ids,
                             ...
                             temperature, top_k, top_p, top_ep, top_ep_tm,
    ...
    while cur_len < max_length:
        ...
        # Add MinLengthLogitsProcessor unconditionally
        if eos_token_id is not None:
            logits_processor.append(
                MinLengthLogitsProcessor(
                    min_length=min_length, eos_token_id=torch.tensor(eos_token_id, device=input_ids.device)
                )
            )
        
        # Add NoRepeatNGramLogitsProcessor only if no_repeat_ngram_size > 0
        ...
        if do_sample:
            ...
            next_token_logscores = src.model_utils.my_top_k_top_p_filtering(
                scores,
                top_k=top_k, top_p=top_p,top_ep=top_ep, top_ep_tm=top_ep_tm,)
            # Sample
        ...
        if eos_token_id is not None:
            ...
            tokens_to_add = next_token * unfinished_sequences + pad_token_id * (1 - unfinished_sequences)

        else:
            tokens_to_add = next_token

        # add token and increase length by one
        input_ids = torch.cat([input_ids, tokens_to_add[:, None]], dim=-1)
        ...
        if eos_token_id is not None:
            ...
            is_sents_unfinished_and_token_to_add_is_eos = unfinished_sequences.mul(eos_in_sents.long()).bool()
            sent_lengths.masked_fill_(is_sents_unfinished_and_token_to_add_is_eos, cur_len)
            # unfinished_senquences is set to zero if eos in sentence
            unfinished_sequences.mul_((~eos_in_sents).long())
...
def create_sample_fn(model, max_len,
                     top_p=1.0, top_k=0, temperature=1.0, top_ep=1, top_ep_tm=1,
                     return_predicted_p=False, cs_alpha=0.0, typical_p=1.0):
    # recalib_fn is applied after top-p/top-k/temp modifications
    
    if cs_alpha > 0:
        # Contrastive search sampling
        fn = lambda prompt: model.generate(
    ...
    elif typical_p != 1.0:  # Use Typical Sampling
        fn = lambda prompt: model.generate(
            input_ids=prompt["input_ids"],
            attention_mask=prompt["attention_mask"],
            max_length=max_len,
            do_sample=True,
            typical_p=typical_p,  # Typical sampling parameter
        )
    else:
        # Original sampling methods
        fn = lambda prompt: generate_text_from_recalibrated_model(
            model, input_ids=prompt["input_ids"],
            max_length=max_len, do_sample=True, temperature=temperature,
            top_k=top_k, top_p=top_p, top_ep=top_ep, top_ep_tm=top_ep_tm
        )
    return fn
...
@torch.no_grad()
def get_samples_from_sample_fn(sampl_fn, ds_tokens, eos_token_id,tokenizer, device, prompt_size=10, batch_size=20):
    outs = []
    input_ids = ds_tokens["input_ids"]
    attention_mask = ds_tokens["attention_mask"]
    b_valid_ds = list(batch_fn(range(len(input_ids)), batch_size))  # Create batches of indices

    for b in tqdm(b_valid_ds):
        batch_input_ids = input_ids[b].to(device)
        batch_attention_mask = attention_mask[b].to(device)

        # Extract the first `prompt_size` non-padding tokens
        effective_prompts = [
            ids[mask.bool()][:prompt_size] for ids, mask in zip(batch_input_ids, batch_attention_mask)
        ]
        
        padded_prompts = torch.stack([
            torch.cat([
                torch.full((prompt_size - len(prompt),), tokenizer.pad_token_id, device=device), prompt
            ]) if len(prompt) < prompt_size and tokenizer.padding_side == "left" else
            torch.cat([
                prompt, torch.full((prompt_size - len(prompt),), tokenizer.pad_token_id, device=device)
            ]) if len(prompt) < prompt_size else prompt
        ...
        prompt = {
            ...
            "attention_mask": (padded_prompts != tokenizer.pad_token_id).long()  # Update attention mask
        }

        sample = sampl_fn(prompt)
        outs.extend(sample.cpu().numpy().tolist())
    
    # Remove EOS tokens added for padding
...
def get_samples_from_sample_fn_saved(sampl_fn, ds_tokens, eos_token_id, device, prompt_size=10, batch_size=20):
    outs = []
    input_ids = ds_tokens["input_ids"]
    attention_mask = ds_tokens["attention_mask"]
    b_valid_ds = list(batch_fn(range(len(input_ids)), batch_size))  # Create batches of indices
    ...
    for b in tqdm(b_valid_ds):
        ...
        #prompt = torch.cat([sen[:, :prompt_size].to(device) for sen in b])
        batch_input_ids = input_ids[b, :prompt_size].to(device)
        batch_attention_mask = attention_mask[b, :prompt_size].to(device)
...
def _generate_no_beam_search_saved_for_later(model, input_ids,
                             cur_len, max_length, min_length, do_sample,
                             temperature, top_k, top_p, top_ep, top_ep_tm,
                             repetition_penalty, no_repeat_ngram_size, bad_words_ids,
                             pad_token_id, eos_token_id, batch_size, attention_mask, use_cache, model_kwargs):
    ...
    while cur_len < max_length:
        ...
        model_inputs = model.prepare_inputs_for_generation(
            input_ids=input_ids, past_key_values=past, attention_mask=attention_mask,
            use_cache=use_cache, cache_position=torch.tensor([cur_len],device=input_ids.device), **model_kwargs
        )
        ...
        #prob_before_filtering = F.softmax(scores, dim=-1)
        ...
        if do_sample:
            ...
            probs = F.softmax(next_token_logscores, dim=-1)
            next_token = torch.multinomial(probs, num_samples=1).squeeze(1)
            # Compute softmax probabilities before and after filtering
    
            #prob_after_filtering = probs
            
            # Sort probabilities in descending order for visualization
            #sorted_prob_before = torch.sort(prob_before_filtering, descending=True)[0].cpu().detach().numpy()
            #sorted_prob_after = torch.sort(prob_after_filtering, descending=True)[0].cpu().detach().numpy()
            
            #total_non_zero = sorted_prob_after[0].sum()
            #sorted_prob_after_non_zero_normalized = sorted_prob_after[0] / total_non_zero
           
            # Ensure the slicing applies correctly for before filtering
            #sorted_prob_before_non_zero = sorted_prob_before[0]
            
            # Create a DataFrame to store the probabilities before and after filtering
            #df = pd.DataFrame({
            #    'Before Filtering': sorted_prob_before_non_zero,
            #    'After Filtering (Normalized)': sorted_prob_after_non_zero_normalized
            #})
            
            # Save the DataFrame to a CSV file
            #file_name = f'probabilities_comparison_normalized_non_zero_p{top_p}_ep{top_ep}_tm{top_ep_tm}.csv'
