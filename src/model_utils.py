...
def my_top_k_top_p_filtering(
    ...
    if top_ep < 1.0:
        ...
        if top_ep_tm == 1.0: 
            ...
            # Element-wise product: p_i * log(p_i)
            p_log_p = sorted_probs * log_probs
            # Sum over the last dimension (dim=-1)
            sum_p_log_p = torch.cumsum(p_log_p, dim=-1)
            #Element-wise product p_i * cum_prob(i)
            p_cum_p = log_probs * cumulative_probs
            # S_tau
            S_tau= sum_p_log_p - p_cum_p
            sorted_indices_to_remove = S_tau > ep
            ...
            cumulative_probs = torch.cumsum(softmax(sorted_logits, dim=-1), dim=-1)
            sorted_probs = softmax(sorted_logits, dim=-1)
            cumulative_probs_tm = torch.cumsum(sorted_probs**(1/tm), dim=-1)
            #Element-wise product p_i * cum_prob_tm(i)
            p_cum_p = sorted_probs**(1-(1/tm)) * cumulative_probs_tm
            # S_tau
            S_tau= cumulative_probs - p_cum_p
            S_tau = S_tau / (1-(1/tm))
            sorted_indices_to_remove = S_tau > ep
            logits = logits/tm
            # scatter sorted tensors to original indexing
...
def reshape_logit_scores(
        scores, temperature=1.0, top_k=0, top_p=1.0, top_ep=1.0, top_ep_tm=1.0,
):
...
def get_tokenwise_log_probs_seq(
        model, sen, temperature=1.0, top_k=0, top_p=1.0, top_ep=1.0, top_ep_tm=1.0,
):
    ...
    logits = reshape_logit_scores(
        outs.logits, temperature, top_k, top_p, top_ep, top_ep_tm
    )
...
@torch.no_grad()
def get_reshaped_log_probs_of_ds(model, ds_tokens, top_p=1.0, top_k=0, top_ep=1.0, top_ep_tm=1.0, temperature=1.0):
    log_probs = []
    device = next(model.parameters()).device
