...
#  time python -u compute_ref_metrics.py --datasplit test --device 0 --parallel_bleu --n_proc_bleu 12 --model_name 'gpt2-large' > outs/ref/test 2>&1
...
def main_metrics(args):
    ...
    ds_tokens["input_ids"] = [
        tokens[tokens != tokenizer.pad_token_id]  # Remove padding tokens
        for tokens in ds_tokens["input_ids"]
    ]
    
    ds_tokens["input_ids"] = [
        tokens[tokens != tokenizer.eos_token_id]  # Remove EOS tokens
        for tokens in ds_tokens["input_ids"]
    ]
    
    ds_tokens = [token.unsqueeze(0) for token in ds_tokens["input_ids"]]
